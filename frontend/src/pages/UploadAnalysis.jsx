...
import { getRequestDetail, listMyRequests, uploadCT } from '../api/client'
...
function UploadAnalysis() {
  ...
  async function handleAnalyze(event) {
    ...
    setError(null)
    try {
      await uploadCT(selectedId, file, scanner)
      const detail = await getRequestDetail(selectedId)
      setResult(detail)
      setFile(null)
      await loadRequests()
    } catch (err) {
    ...
    <div className="flex h-screen overflow-hidden bg-bg text-text">
      ...
      <main className="h-screen min-h-0 min-w-0 flex-1 overflow-y-auto">
        ...
        <div className="p-8">
          ...
          <section className="mb-4 flex items-center justify-between gap-5 rounded-[14px] border border-border bg-panel p-5 shadow-[0_10px_28px_rgba(26,52,73,0.06)]"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-lg bg-accent-soft text-accent"><FileCheck size={19} /></div><div><p className="text-[10px] uppercase tracking-wide font-bold text-text-muted">Case assignment</p><p className="text-sm font-semibold">Select the patient record for this scan</p><p className="text-xs text-text-muted mt-1">The original image is sent to the analysis service.</p></div></div><select value={selectedId} onChange={(event) => { setSelectedId(event.target.value); setResult(null); setFile(null) }} className="w-full max-w-xs rounded-lg border border-border bg-bg px-3 py-2.5 text-sm outline-none focus:border-accent"><option value="">Select a request</option>{availableRequests.map((request) => <option key={request.id} value={request.id}>{request.id}</option>)}</select></section>
          <div className="grid grid-cols-1 gap-5 xl:grid-cols-[.8fr_1.2fr]">
            <form onSubmit={handleAnalyze} className="rounded-[14px] border border-border bg-panel p-5 shadow-[0_10px_28px_rgba(26,52,73,0.06)]"><div className="flex items-start justify-between"><div><h3 className="text-base font-semibold">Upload CT image</h3><p className="mt-1 text-sm text-text-muted">{selectedRequest?.id || 'Choose a request above'}</p></div><Upload size={19} className="text-text-muted" /></div><input value={scanner} onChange={(event) => setScanner(event.target.value)} placeholder="Scanner" className="mt-5 w-full rounded-lg border border-border bg-bg px-3 py-2.5 text-sm outline-none focus:border-accent" /><input ref={inputRef} type="file" accept=".nii,.nii.gz,.dcm" onChange={(event) => setFile(event.target.files?.[0] || null)} className="hidden" /><button type="button" onClick={() => inputRef.current?.click()} className="mt-4 flex min-h-48 w-full cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-border bg-bg px-6 text-center hover:border-accent hover:bg-accent-soft/30 transition">{file ? <><FileCheck size={28} className="mb-3 text-flag-negative" /><p className="text-sm font-semibold truncate max-w-full">{file.name}</p><p className="mt-1 text-xs text-text-muted">Click to change file</p></> : <><FileUp size={28} className="mb-3 text-accent" /><p className="text-sm font-semibold">Drop scan here or browse</p><p className="mt-1 text-xs text-text-muted">Original image only · NII, NII.GZ, DCM</p></>}</button><button type="submit" disabled={!file || !selectedId || submitting} className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-accent px-4 py-3 text-sm font-semibold text-white hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-40"><Activity size={16} /> {submitting ? 'Analyzing...' : 'Analyze scan'}</button></form>
            <section className="min-h-[430px] rounded-[14px] border border-border bg-panel p-6 shadow-[0_10px_28px_rgba(26,52,73,0.06)] grid place-items-center">{result ? <div className="w-full max-w-md text-center"><div className={`mx-auto grid h-12 w-12 place-items-center rounded-xl ${result.prediction >= 0.5 ? 'bg-flag-positive-bg text-flag-positive' : 'bg-flag-negative-bg text-flag-negative'}`}><Activity size={24} /></div><p className="mt-4 text-[11px] uppercase tracking-wide font-bold text-text-muted">AI-assisted prediction</p><h3 className={`font-display text-2xl font-extrabold mt-2 ${result.prediction >= 0.5 ? 'text-flag-positive' : 'text-flag-negative'}`}>{result.prediction >= 0.5 ? 'PDAC likely' : 'PDAC unlikely'}</h3><p className="font-display text-4xl font-extrabold mt-2">{(result.prediction * 100).toFixed(1)}%</p><p className="text-sm text-text-muted mt-3">Analysis complete. The request is ready for radiologist review.</p></div> : <div className="text-center"><Activity size={28} className="mx-auto text-accent" /><h3 className="font-display text-xl font-extrabold mt-4">Your analysis will appear here</h3><p className="max-w-sm text-sm text-text-muted mt-2">Upload a valid CT image to begin AI-assisted analysis.</p></div>}</section>
          </div>
        </div>
      </main>
    </div>
